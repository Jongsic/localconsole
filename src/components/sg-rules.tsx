...
export function RuleTable({ title, rules }: { title: string; rules: Ec2SgRule[] }) {
  ...
  return (
    <div>
      <div className="mb-1.5 text-[11px] font-medium uppercase text-slate-400">{title}</div>
      {rules.length === 0 ? (
        <p className="text-sm text-slate-400">{t("ec2.security.noRules")}</p>
      ) : (
        <table className="w-full text-left text-xs">
          <thead className="text-[10px] uppercase text-slate-400">
            <tr>
              <th className="py-1 pr-3 font-medium">{t("ec2.security.protocol")}</th>
              <th className="py-1 pr-3 font-medium">{t("ec2.security.ports")}</th>
              <th className="py-1 font-medium">{t("ec2.security.source")}</th>
            </tr>
          </thead>
          <tbody>
            {rules.map((r, i) => (
              ...
              <tr key={i} className="border-t border-slate-100">
                <td className="py-1 pr-3 text-slate-600">{protocol(r.protocol)}</td>
                <td className="py-1 pr-3 text-slate-600">{ports(r)}</td>
                <td className="py-1 font-mono text-slate-500">{r.source}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <div>
      <div className="mb-2 flex items-baseline gap-2">
        ...
        <span className="font-mono text-xs text-slate-400">{group.groupId}</span>
